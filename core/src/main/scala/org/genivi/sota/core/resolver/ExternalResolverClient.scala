...
trait ExternalResolverClient {
  ...
  def resolve(namespace: Namespace, packageId: PackageId): Future[Map[(Namespace, Device.DeviceId), Set[PackageId]]]
...
class DefaultExternalResolverClient(baseUri : Uri, resolveUri: Uri, packagesUri: Uri, devicesUri: Uri)
  ...
  override def resolve(namespace: Namespace, packageId: PackageId): Future[Map[(Namespace, Device.DeviceId), Set[PackageId]]] = {
    implicit val responseDecoder : Decoder[Map[Device.DeviceId, Set[PackageId]]] =
      Decoder[Seq[(Device.DeviceId, Set[PackageId])]].map(_.toMap)
    ...
    request(packageId).flatMap { response =>
      Unmarshal(response.entity).to[Map[Device.DeviceId, Set[PackageId]]].map { parsed =>
        parsed.map { case (k, v) => (namespace, k) -> v }
      }
    }.recover { case _ => Map.empty[(Namespace, Device.DeviceId), Set[PackageId]] }
  }
