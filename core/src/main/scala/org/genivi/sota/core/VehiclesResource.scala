    ...
    parameters(('status.?(false), 'regex.?)) { (includeStatus: Boolean, regex: Option[String]) =>
      if (includeStatus) {
        ...
        val devicesWithStatus = devicesAndVinsWithStatus.map { case (devices, vinsWithStatus) => for {
          d <- devices
          status = vinsWithStatus.filter(_.vin.get == d.deviceId).headOption.map(_.status).getOrElse(VehicleStatus.NotSeen)
        ...
        onComplete(devicesWithStatus) {
          ...
          case Failure(ex) => complete((StatusCodes.InternalServerError, "error: cannot lookup update status for devices"))
        }
      } else {
