...
class GroupsResourceSpec extends FunSuite
  ...
  def listDevicesInGroup(groupName: Name)(implicit ec: ExecutionContext): HttpRequest =
    Get(Resource.uri("device_groups").withQuery(Query("groupName" -> groupName.get)))
  ...
  test("creating groups is possible") {
    ...
    listDevicesInGroup(groupName) ~> route ~> check {
      status shouldBe OK
